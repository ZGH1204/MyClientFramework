...
public class ExcelUtility
{
    ...
    /// 转换为CSharp
    /// </summary>
    /// <param name="cSharpPath">CSharp文件路径</param>
    public void ConvertToCSharp(string cSharpPath, Encoding encoding)
    {
        ...
        for (int i = 0; i < colCount; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                col.Add(mSheet.Rows[j][i].ToString());
            }
            //添加到表数据中
            table.Add(col);
        }
        ...
        using (FileStream fileStream = new FileStream(cSharpPath, FileMode.Create, FileAccess.Write))
        {
            using (TextWriter textWriter = new StreamWriter(fileStream, encoding))
            {
                ...
                string[] path = fileStream.Name.Split('/');
                stringBuilder.Append("public class " + path[path.Length - 1].Replace(".cs", "") + "\n{");
                foreach (var list in table)
                {
                    ...
                    stringBuilder.Append("\n\t/// <summary>");
                    string type = list[1];
                    if (list[1].Equals("string[]"))
                    {
                        type = "List<string>";
                    }
                    else if (list[1].Equals("int[]"))
                    {
                        type = "List<int>";
                    }
                    else if (list[1].Equals("bool[]"))
                    {
                        type = "List<bool>";
                    }
                    else if (list[1].Equals("float[]"))
                    {
                        type = "List<float>";
                    }
                    else if (list[1].Equals("Vector2[]"))
                    {
                        type = "List<Vector2>";
                    }
                    stringBuilder.Append("\n\tpublic " + type + " " + list[0] + " { get; set; }\n");
                }
                stringBuilder.Append("}");
