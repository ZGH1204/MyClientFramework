...
namespace ZFramework.Network
{
    internal partial class NetworkManager
    {
        ...
        private sealed partial class NetworkChannel : INetworkChannel, IDisposable
        {
            ...
            public Action<NetworkChannel, object> NetworkChannelConnected { get; set; }
            public Action<NetworkChannel> NetworkChannelClosed { get; set; }
            public Action<NetworkChannel, int, object> NetworkChannelSended { get; set; }
            public Action<NetworkChannel, int> NetworkChannelMissHeartBeat { get; set; }
            public Action<NetworkChannel, NetworkErrorCode, string> NetworkChannelError { get; set; }
            public Action<NetworkChannel, object> NetworkChannelCustomError { get; set; }
            ...
            public void Connect(IPAddress ipAddress, int port, object userData)
            {
                ...
                try
                {
                    m_Socket.BeginConnect(ipAddress, port, this.ConnectCallback, new SocketUserData(m_Socket, userData));
                }
            ...
            public void Send(byte[] buffer, int offset, int size, object userData)
            {
                ...
                try
                {
                    m_Socket.BeginSend(buffer, offset, size, SocketFlags.None, this.SendCallback, new SocketUserData(m_Socket, userData));
                }
                catch (System.Exception exception)
                {
                    m_Active = false;
            ...
            private void Receive()
            {
                try
                {
                    m_Socket.BeginReceive(m_ReceiveState.Stream.GetBuffer(), (int)m_ReceiveState.Stream.Position, (int)(m_ReceiveState.Stream.Length - m_ReceiveState.Stream.Position), SocketFlags.None, this.ReceiveCallback, m_Socket);
                }
                catch (System.Exception exception)
                {
                    m_Active = false;
